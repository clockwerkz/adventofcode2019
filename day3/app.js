...
let steps = 0;
let directionsToProcess = wire1.length > 0;
while (directionsToProcess) {
    ...
    if (distance === 0) {
        if (wire1.length > 0) {
            ...
            distance = newDirection.distance;
        } 
    }
    if (wire1.length <= 0) {
    ...
    if (directionsToProcess) {
        if (distance > 0) {
          let pair = movePointer(x,y, direction);
          x = pair[0];
          y = pair[1];
          cache[convertToString(pair)] = steps++;
          distance--;
        }
    }
} 
...
directionsToProcess = wire2.length > 0;
direction = null;
distance = 0;
x = 0;
y = 0;
...
while (directionsToProcess) {
    ...
    if (directionsToProcess) {
        if (distance > 0) {
          ...
          x = pair[0];
          y = pair[1];
          if (cache[convertToString(pair)]) {
            console.log('Intersection at', [x,y], "total steps:", cache[convertToString(pair)]+steps);
            console.log("first wirse steps:", cache[convertToString(pair)]+steps, "second wire steps:", steps);
            const newDistance = Math.abs(x) + Math.abs(y);
            closestIntersection = (closestIntersection > newDistance) ? newDistance : closestIntersection;
          }
          distance--;
        }
    }
...
function convertToString(arrayPair) {
  return arrayPair[0] + ',' + arrayPair[1];
}
...
function movePointer(x, y, direction) {
    switch (direction) {
        case 'R':
            x += 1;
            break;
